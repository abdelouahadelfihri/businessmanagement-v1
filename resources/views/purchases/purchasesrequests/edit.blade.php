            ...
            <div class="card-body">
                ...
                <form action="{{ route('purchases.purchasesrequests.update', $purchaseRequest) }}" method="POST"
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" value="{{ optional($purchaseRequest->requestedBy)->name }}"
                            readonly>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="priority" class="form-control" required>
                                <option value="low" @selected(old('priority', $purchaseRequest->priority) == 'low')>Low
                                </option>
                                <option value="medium" @selected(old('priority', $purchaseRequest->priority) == 'medium')>
                                    Medium</option>
                                <option value="high" @selected(old('priority', $purchaseRequest->priority) == 'high')>High
                                </option>
                                <option value="urgent" @selected(old('priority', $purchaseRequest->priority) == 'urgent')>
                                    Urgent</option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="status" class="form-control" required>
                                ...
                                <option value="pending" @selected(old('status', $purchaseRequest->status) == 'pending')>
                                    Pending</option>
                                <option value="approved" @selected(old('status', $purchaseRequest->status) == 'approved')>
                                    Approved</option>
                                <option value="rejected" @selected(old('status', $purchaseRequest->status) == 'rejected')>
                                    Rejected</option>
                                <option value="ordered" @selected(old('status', $purchaseRequest->status) == 'ordered')>
                                    Ordered</option>
                                <option value="completed" @selected(old('status', $purchaseRequest->status) == 'completed')>
                                    Completed</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered align-middle" id="linesTable">
                                ...
                                <tbody id="linesBody">
                                    {{-- rows injected by JS from existing lines + on Add --}}
                                </tbody>
                                <tfoot>
                                    <tr>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="notes" class="form-control"
                            rows="2">{{ old('notes', $purchaseRequest->notes) }}</textarea>
                    </div>
...
@push('scripts')
    <script>
        (function () {
            ...
            const existingLines = @json($purchaseRequest->lines->map(function ($line) {
                return [
            ...
            // Load existing lines, or start with one empty row if none exist
